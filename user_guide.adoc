= lib60870-C 2.0 User Guide
Michael Zillgith

== Introduction

lib60870-C is a new implementation of the IEC 60870-5 protocol for client (master station) and server (controlled station). The library implements all data types of the IEC 60870-5-101/104 specifications. *lib60870-C* is implemented in standard C and is compatible with the C99 standard. It is designed to be as easy to use as possible.
...
A new connection is simple created by calling a the _create_ function of the CS104_Connection type:
...
After the connection object is created you can now simply call the _connect_ function to connect to the server:
...
This call is non-blocking. You have to evaluate the response in the ASDUReceivedHandler callback function.
...
After the server is configured it can be started with the _start_ function

  CS104_Slave_start(slave);

To deactivate the IEC 60870-5-104 service the server can be stopped with the _stop_ function.

  CS104_Slave_stop(slave);

=== CS101 (serial) slave configuration and setup

Analog to the master side the CS101 slave side can be configured for one of the two modes (_balanced_ or _unbalanced_).
